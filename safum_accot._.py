...
# فتح رابط مجموعة الواتساب عند التشغيل
webbrowser.open('https://chat.whatsapp.com/H97JQBnTeC57NKw7Hg8PkK')

created = 0
failed = 0

# تعديل نص طلب الأيدي
id = input(f' أسامة يريد الأيدي 🤖 : ')
print(('—' * 25) + '\n• جيكو اليوتيوبر •\n' + ('—' * 25))
os.system('clear')
...
def create():
    ...
    user = str(random.choice(ch)) + str(''.join(random.choice(ch) for i in range(9)))

    # النص بتنسيق كبير (Bold) ومسافات للتوسيط
    tlg = f'''
**تم صيد حساب بواسطة/سافيوم حسابات⇣**
**❏────────✆────────❏**

   **˛ الحسـاب الجديـد ☎️ : `{user}`**
   **˛ البـاسـورد 🔐 : `hhhh`**
...
'''
    ...
    try:
        ...
        if '"status":"Success"' in str(decoded_data):
            created += 1
            # إرسال الرسالة مع تفعيل نمط Markdown للخط العريض والتوسيط
            requests.post(f"https://api.telegram.org/bot{token}/sendmessage", 
                          data={'chat_id': id, 'text': tlg, 'parse_mode': 'Markdown'})
        elif '"comment":"Exists"' in str(decoded_data) or '"comment":"Retry"' in str(decoded_data):
            failed += 1
        ws.close()
    except:
        failed += 1
